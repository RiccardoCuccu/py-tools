...
**Purpose:** `social_media_tracker.py` is a script designed to automatically track follower counts across Instagram and YouTube on a configurable schedule. The script runs continuously and checks follower numbers on the last day of each month (or weekly) at a specified time, logging all data to an Excel file with historical tracking and change calculations.
...
- Follower data is scraped from public profile pages without requiring authentication or API keys.
...
```
pip install requests schedule openpyxl instaloader
```
